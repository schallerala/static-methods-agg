...
public class QDoxFactoryReader implements Iterable<FactoryMethod> {
    ...
    public Iterator<FactoryMethod> iterator() {
        ...
        for (JavaMethod jm : classSource.getMethods()) {
            if ( ! (jm.isPublic() && jm.isStatic() && ! jm.getReturnType().getGenericFullyQualifiedName().equals("void"))) {
                continue;
            }
        ...
        Collections.sort(methods, Comparator.comparing(FactoryMethod::getName));
